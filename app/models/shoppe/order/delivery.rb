module Shoppe
  class Order < Shoppe::ApplicationRecord
    ...
    before_save do
      if received? && (saved_change_to_delivery_service_id? || (self.saved_change_to_delivery_price? && read_attribute(:delivery_price).blank?))
    ...
    def delivery_service_prices
      if delivery_required?
        ...
        prices = prices.select { |p| p.states.empty? || p.state?(self.delivery_address4) } if self.delivery_country.code2 == 'US'
    ...
    def delivery_service_price
      self.delivery_service && self.delivery_service.delivery_service_prices.where(currency: currency).for_weight(self.total_weight).first
    end
