module Shoppe
  class ProductsController < Shoppe::ApplicationController
    ...
    before_action { params[:id] && @product = Shoppe::Product.root.find(params[:id]) }
    ...
    def safe_params
      ...
      params[:product].permit(:name, :sku, :permalink, :description, :short_description, :weight, :price, :cost_price, :tax_rate_id, :stock_control, :active, :featured, :in_the_box, :attachments => [:default_image => file_params, :data_sheet => file_params, :extra => file_params], :product_attributes_array => [:key, :value, :searchable, :public], :product_category_ids => [], :product_prices_array => [:currency, :price])
    end
