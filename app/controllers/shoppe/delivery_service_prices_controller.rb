module Shoppe
  class DeliveryServicePricesController < Shoppe::ApplicationController
    ...
    before_action { @delivery_service = Shoppe::DeliveryService.find(params[:delivery_service_id])}
    ...
    def safe_params
      params[:delivery_service_price].permit(:price, :cost_price, :currency, :tax_rate_id, :min_weight, :max_weight, :name, :code, :country_ids => [], :state_codes => [])
    end
